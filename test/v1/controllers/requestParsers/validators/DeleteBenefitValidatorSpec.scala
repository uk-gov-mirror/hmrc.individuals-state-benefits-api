...
class DeleteBenefitValidatorSpec extends UnitSpec {
  ...
  class Test extends MockCurrentDateTime with MockAppConfig {
    ...
    val validator = new DeleteBenefitValidator()
  ...
  "running a validation" should {
    "return no errors" when {
      "a valid request is supplied" in new Test {
        validator.validate(DeleteBenefitRawData(validNino, validTaxYear, validBenefitId)) shouldBe Nil
      }
    }

    "return NinoFormatError error" when {
      "an invalid nino is supplied" in new Test {
        validator.validate(DeleteBenefitRawData("A12344A", validTaxYear, validBenefitId)) shouldBe
    ...
    "return TaxYearFormatError error" when {
      "an invalid tax year is supplied" in new Test {
        validator.validate(DeleteBenefitRawData(validNino, "20178", validBenefitId)) shouldBe
    ...
    "return BenefitIdFormatError error" when {
      "an invalid benefit ID is supplied" in new Test {
        validator.validate(DeleteBenefitRawData(validNino, validTaxYear, "ABCDE12345FG")) shouldBe
    ...
    "return RuleTaxYearNotSupportedError error" when {
      "a tax year that is not supported is supplied" in new Test {
        validator.validate(DeleteBenefitRawData(validNino, "2018-19", validBenefitId)) shouldBe
    ...
    "return RuleTaxYearRangeInvalidError error" when {
      "an out of range tax year is supplied" in new Test {
        validator.validate(DeleteBenefitRawData(validNino, "2020-22", validBenefitId)) shouldBe
    ...
    "return NinoFormatError and TaxYearFormatError errors" when {
      "request supplied has invalid nino and tax year" in new Test {
        validator.validate(DeleteBenefitRawData("A12344A", "20199", validBenefitId)) shouldBe
    ...
    "return NinoFormatError, TaxYearFormatError and BenefitIdFormatError errors" when {
      "request supplied has invalid nino, tax year and benefit ID" in new Test {
        validator.validate(DeleteBenefitRawData("A12344A", "20178", "ABCDE12345FG")) shouldBe
