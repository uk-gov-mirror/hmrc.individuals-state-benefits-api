...
class UpdateBenefitAmountsValidatorSpec extends UnitSpec with ValueFormatErrorMessages {
  ...
  class Test extends MockCurrentDateTime with MockAppConfig {
    ...
    val validator = new UpdateBenefitAmountsValidator()
  ...
  "UpdateBenefitAmountsValidator" when {
    "running a validation" should {
      "return no errors for a valid request" in new Test {
        validator.validate(UpdateBenefitAmountsRawData(validNino, validTaxYear, validBenefitId, validRawBody)) shouldBe Nil
      }

      // parameter format error scenarios
      "return NinoFormatError error when the supplied NINO is invalid" in new Test {
        validator.validate(UpdateBenefitAmountsRawData("A12344A", validTaxYear, validBenefitId, validRawBody)) shouldBe
          List(NinoFormatError)
      }

      "return TaxYearFormatError error for an invalid tax year format" in new Test {
        validator.validate(UpdateBenefitAmountsRawData(validNino, "20199", validBenefitId, validRawBody)) shouldBe
          List(TaxYearFormatError)
      }

      "return BenefitIdFormatError error for an invalid benefit ID" in new Test {
        validator.validate(UpdateBenefitAmountsRawData(validNino, validTaxYear, "ABCDE12345FG", validRawBody)) shouldBe
          List(BenefitIdFormatError)
      }

      "return RuleTaxYearRangeInvalidError error for an invalid tax year range" in new Test {
        validator.validate(UpdateBenefitAmountsRawData(validNino, "2020-22", validBenefitId, validRawBody)) shouldBe
          List(RuleTaxYearRangeInvalidError)
      }

      "return multiple errors for multiple invalid request parameters" in new Test {
        validator.validate(UpdateBenefitAmountsRawData("A12344A", "2020-22", "ABCDE12345FG", validRawBody)) shouldBe
      ...
      "return RuleTaxYearNotSupportedError error for an unsupported tax year" in new Test {
        validator.validate(UpdateBenefitAmountsRawData(validNino, "2019-20", validBenefitId, validRawBody)) shouldBe
          List(RuleTaxYearNotSupportedError)
      }

      "return RuleTaxYearNotEndedError error for a tax year which hasn't ended" in new Test {
        validator.validate(UpdateBenefitAmountsRawData(validNino, "2022-23", validBenefitId, validRawBody)) shouldBe
      ...
      "return RuleIncorrectOrEmptyBodyError error for an empty request body" in new Test {
        validator.validate(UpdateBenefitAmountsRawData(validNino, validTaxYear, validBenefitId, emptyRawBody)) shouldBe
          List(RuleIncorrectOrEmptyBodyError)
      }

      "return RuleIncorrectOrEmptyBodyError error for a non-empty JSON body with no mandatory fields provided" in new Test {
        validator.validate(UpdateBenefitAmountsRawData(validNino, validTaxYear, validBenefitId, missingMandatoryFieldRawRequestBody)) shouldBe
      ...
      "return RuleIncorrectOrEmptyBodyError error for an incorrect request body" in new Test {
        validator.validate(UpdateBenefitAmountsRawData(validNino, validTaxYear, validBenefitId, incorrectFormatRawBody)) shouldBe
      ...
      "return ValueFormatError error for incorrect field formats" in new Test {
        validator.validate(UpdateBenefitAmountsRawData(validNino, validTaxYear, validBenefitId, allInvalidValueRawRequestBody)) shouldBe
          List(
