...
class UpdateBenefitValidatorSpec extends UnitSpec {
  ...
  class Test extends MockCurrentDateTime with MockAppConfig {
    ...
    val validator = new UpdateBenefitValidator()
  ...
  "UpdateBenefitValidator" when {
    "running a validation" should {
      "return no errors for a valid request" in new Test {
        validator.validate(UpdateBenefitRawData(validNino, validTaxYear, benefitId, validRawBody)) shouldBe Nil
      }

      // parameter format error scenarios
      "return NinoFormatError error when the supplied NINO is invalid" in new Test {
        validator.validate(UpdateBenefitRawData("A12344A", validTaxYear, benefitId, validRawBody)) shouldBe
          List(NinoFormatError)
      }

      "return TaxYearFormatError error for an invalid tax year format" in new Test {
        validator.validate(UpdateBenefitRawData(validNino, "20178", benefitId, validRawBody)) shouldBe
          List(TaxYearFormatError)
      }

      "return RuleTaxYearRangeInvalidError error for an invalid tax year range" in new Test {
        validator.validate(UpdateBenefitRawData(validNino, "2018-20", benefitId, validRawBody)) shouldBe
          List(RuleTaxYearRangeInvalidError)
      }

      "return multiple errors for multiple invalid request parameters" in new Test {
        validator.validate(UpdateBenefitRawData("notValid", "2018-20", benefitId, validRawBody)) shouldBe
      ...
      "return RuleTaxYearNotSupportedError error for an unsupported tax year" in new Test {
        validator.validate(UpdateBenefitRawData(validNino, "2018-19", benefitId, validRawBody)) shouldBe
          List(RuleTaxYearNotSupportedError)
      }

      "return RuleTaxYearNotEndedError error for a tax year which hasn't ended" in new Test {
        validator.validate(UpdateBenefitRawData(validNino, "2022-23", benefitId, validRawBody)) shouldBe
      ...
      "return RuleIncorrectOrEmptyBodyError error for an empty request body" in new Test {
        validator.validate(UpdateBenefitRawData(validNino, validTaxYear, benefitId, emptyRawBody)) shouldBe
      ...
      "return RuleIncorrectOrEmptyBodyError error for an incorrect request body" in new Test {
        ...
        validator.validate(UpdateBenefitRawData(validNino, validTaxYear, benefitId, noMandatoryFieldsRawBody)) shouldBe
      ...
      "return multiple errors for incorrect field formats" in new Test {
        validator.validate(UpdateBenefitRawData(validNino, validTaxYear, benefitId, invalidDateTypeRawBody)) shouldBe
      ...
      "return multiple errors for dates which precede the current tax year and are incorrectly ordered" in new Test {
        validator.validate(UpdateBenefitRawData(validNino, validTaxYear, benefitId, startDateBeforeEndDateRawBody1)) shouldBe
      ...
      "return multiple errors for dates which exceed the current tax year and are incorrectly ordered" in new Test {
        validator.validate(UpdateBenefitRawData(validNino, validTaxYear, benefitId, incorrectDatesRawBody2)) shouldBe
