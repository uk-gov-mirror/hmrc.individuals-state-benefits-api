...
import v1.mocks.requestParsers.MockUpdateBenefitAmountsRequestParser
import v1.mocks.services.{MockEnrolmentsAuthService, MockMtdIdLookupService, MockUpdateBenefitAmountsService}
...
class UpdateBenefitAmountsControllerSpec
  extends ControllerBaseSpec
    with MockEnrolmentsAuthService
    with MockMtdIdLookupService
    with MockAppConfig
    with MockUpdateBenefitAmountsService
    with MockUpdateBenefitAmountsRequestParser {

  trait Test {
    val hc = HeaderCarrier()

    val controller = new UpdateBenefitAmountsController(
      authService = mockEnrolmentsAuthService,
      lookupService = mockMtdIdLookupService,
      appConfig = mockAppConfig,
      requestParser = mockUpdateBenefitAmountsRequestParser,
  ...
  "UpdateBenefitAmountsController" should {
    "return OK" when {
      "happy path" in new Test {
        ...
        MockUpdateBenefitAmountsRequestParser
          .parse(rawData)
        ...
        val result: Future[Result] = controller.updateBenefitAmounts(nino, taxYear, benefitId)(fakePutRequest(requestBodyJson))
