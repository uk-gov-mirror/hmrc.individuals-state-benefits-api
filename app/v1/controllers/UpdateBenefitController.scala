...
@Singleton
class UpdateBenefitController @Inject()(val authService: EnrolmentsAuthService,
  ...
  implicit val endpointLogContext: EndpointLogContext =
    EndpointLogContext(
      controllerName = "UpdateBenefitController",
      endpointName = "UpdateBenefitAmounts"
    )

  def update(nino: String, taxYear: String, benefitId: String): Action[JsValue] =
    authorisedAction(nino).async(parse.json) { implicit request =>

      val rawData = UpdateBenefitRawData(
        nino = nino,
        taxYear = taxYear,
        benefitId = benefitId,
      ...
      val result =
        for {
          ...
          Ok(updateBenefitHateoasBody(appConfig, nino, taxYear, benefitId))
